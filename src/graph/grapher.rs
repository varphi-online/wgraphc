...
const BUF_SIZE: usize = ((2 * GRAPH_RESOLUTION.pow(2) - 2) * 8) as usize;
...
pub fn faster_call(
    ...
    unsafe {
        ...
        let mut i = 0;
        clog!("{:?}", apply_transformation(0.0, 3.0));
        for y in (0..GRAPH_RESOLUTION).rev() {
            ...
            for x in 0..GRAPH_RESOLUTION {
                ...
                if (i < BUF_SIZE) && i % 2 == 0 {
                    let (output_real, output_imag) = apply_transformation(input_real, input_imag);
                    [BUFFER[i], BUFFER[i + 1]] = match (x_axis.as_str(), y_axis.as_str()) {
                        ("i_r", "o_r") => to_screenspace(input_real, output_real),
                        ("i_r", "o_i") => to_screenspace(input_real, output_imag),
                        ("o_r", "o_i") => to_screenspace(output_real, output_imag),
                        ("i_i", "o_i") => to_screenspace(input_imag, output_imag),
                        ("i_i", "o_r") => to_screenspace(input_imag, output_real),
                        ("o_i", "o_r") => to_screenspace(output_imag, output_real),
                        _ => [0.0, 0.0],
                    }
                }
                i += 2;
