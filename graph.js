...
document.addEventListener('DOMContentLoaded', (event) => {
  wasm.ensureWasmInit().then(() => {
    ...
    async function render() {
      ...
      let values = await wasm.squaredvals(graph.bounds, canvas.width, canvas.height,true);

      let pts = Array.from(values.filter((value, index) => index % 2 == 0));
      
      //drawCurve(cnv,pts);
      let points = [];
      for (let i = 0; i < Math.sqrt(pts.length*1.8); i += 2) {
